...
from .separate_optimization import step_separate,clipped_value_loss
...
def main():
    ...
    e=OceanTrainingPool(ROOT/a.inventory)
    c=e.cfg
    if (a.particles,a.n_targets,a.coverage_mode)!=(c.particles,c.n_targets,c.coverage_mode):
        ap.error("CLI environment settings must match the frozen inventory config")
    field_path=Path(e.field.path)
    ...
    actor_parameters=list(model.actor.parameters())+[model.logstd]
    critic_parameters=list(model.critic.parameters())
    opt=torch.optim.Adam(actor_parameters,lr=3e-4,eps=1e-5)
    critic_opt=torch.optim.Adam(critic_parameters,lr=3e-4,eps=1e-5)
    metadata={"version":"mappo_separate_optim_v1","status":"feed-forward MAPPO refinement; no value normalization or recurrent-state equivalence claim",
              "optimization":"separate actor/critic Adam and gradient clipping; clipped raw-value loss",
    ...
    for update in range(a.updates):
        ...
        for epoch in range(a.epochs):
            for idx in torch.randperm(a.rollout,device=device).split(a.minibatch):
                ...
                value_loss=clipped_value_loss(model.critic(state[idx]).squeeze(-1),old_values[idx],targets[idx])
                latent_entropy=torch.distributions.Normal(mu,ls.exp()).entropy().sum(-1).mean()
                actor_objective=actor_loss-a.entropy_coef*latent_entropy
                loss=actor_objective+value_loss
                if not torch.isfinite(loss): raise RuntimeError("Non-finite training loss")
                step_separate(actor_objective,value_loss,actor_parameters,critic_parameters,opt,critic_opt)
        ...
        payload={"model":model.state_dict(),"optimizer":opt.state_dict(),"critic_optimizer":critic_opt.state_dict(),"metadata":metadata,
