...
def main():
    ...
    ap.add_argument("--data",default="runs/real-east-china-2018-01-utc-v2/segments.npz")
    ap.add_argument("--snapshot-every",type=int,default=50)
    ap.add_argument("--manifest",default="runs/real-east-china-2018-01-utc-v2/split-manifest.json")
    ap.add_argument("--out",required=True)
    a=ap.parse_args()
    if a.transport_aux and (a.peva_aux or not a.real_data):
        ap.error("transport auxiliary requires --real-data and excludes legacy --peva-aux")
    if a.guidance != 0:
    ...
    c=replace(Config(),particles=a.particles,n_targets=1 if a.real_data else 3,coverage_mode=a.coverage_mode)
    data=(ROOT/a.data).resolve(); manifest=(ROOT/a.manifest).resolve()
    e=RealMaritimeSAR(data,c,split="train",manifest_path=manifest) if a.real_data else MaritimeSAR(c)
    ...
    metadata={"version":"mappo_v2","status":"validation/pilot until convergence and held-out evaluation",
              "args":vars(a),"config":e.config_dict(),"device":str(device),
              "parameters":sum(p.numel() for p in model.parameters()),
              "manifest_sha256":file_sha(manifest) if a.real_data else None,
              "data_sha256":file_sha(data) if a.real_data else None,
    ...
    for update in range(a.updates):
        ...
        adv,returns=gae(buf["r"],buf["v"],buf["nv"],buf["boot"],buf["cont"])
