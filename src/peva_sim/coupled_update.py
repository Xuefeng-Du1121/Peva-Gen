"""One on-policy, value-prioritized coupled update (paper Sections 4.2-4.5).
...
def critic_latent_gradient(critic,state,n_uavs,create_graph=False):
    # Critic target coordinates are x/L; belief latent is z=2*x/L-1.
    # d(x/L)/dz = 1/2, not 2 and not L/2.
    if state.ndim!=2 or state.shape[1]!=2*n_uavs+4:
        raise ValueError("single-target critic state contract mismatch")
    s=state.detach().requires_grad_(True)
    value=critic(s).squeeze(-1)
    gradient=torch.autograd.grad(value.sum(),s,create_graph=create_graph)[0]
    return value,gradient[:,2*n_uavs:2*n_uavs+2]/2
...
class CoupledUpdater:
    def __init__(self,policy,auxiliary,policy_optimizer,auxiliary_optimizer,
                 n_uavs,seed=0,beta_min=.1,risk_lambda=1.,alignment_weight=.01):
        self.policy=policy;self.auxiliary=auxiliary
        if not 0<=beta_min<=1 or risk_lambda<0 or alignment_weight<0:
    ...
    def update(self,rollout,batch_size):
        ...
        _,cg=critic_latent_gradient(self.policy.critic,r["state"],self.n_uavs)
        if (r["spread"]<0).any():
        ...
        gate=1-torch.tanh(self.risk_lambda*spread)
        risk_weight=corr.clamp(.1,10)*gate
        surrogate=torch.minimum(ratio*advantage,ratio.clamp(.8,1.2)*advantage).mean(-1)
        actor_loss=-(risk_weight*surrogate).mean()
        value,critic_grad=critic_latent_gradient(self.policy.critic,r["state"][idx],self.n_uavs,create_graph=True)
