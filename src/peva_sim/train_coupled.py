...
from .physics_priority import latent_gradient
from .ppo_math import disk_action,latent_log_prob,gae
from .coupled_update import CoupledUpdater
...
def main():
    ...
    if adapter.metadata.get("diffusion_parameterization")!="v_residual":
        ap.error("legacy numerically unstable auxiliary architecture not supported")
    ...
    aopt=torch.optim.Adam(adapter.model.parameters(),lr=3e-4)
    ...
    metadata={"version":"coupled_transport_pilot_v1","status":"coupled update pilot; not complete PEVA-Gen",
              ...
              "latent":"single current target, affine centered-region coordinates; neural decoder not implemented",
    ...
    try:
        for update in range(a.updates):
            ...
            for _ in range(a.rollout):
                ...
                pg=latent_gradient(env,env.targets)[0]
